...
const unsigned long AIRLOCK_REQUEST_RETRY_MS = 200;
...
bool waitingForAirlockOpen = false;
...
const uint8_t SPECIAL_DETECT_FRAMES = 2;
const uint8_t RIGHT_ANGLE_DETECT_FRAMES = 2;
...
void clearAirlockWait() {
  pendingAirlockUid = "";
  waitingForAirlockOpen = false;
  lastAirlockSendAttemptMs = 0;
}

void resumeAfterAirlockOpen(const char *sourceName) {
  if (!waitingForAirlockOpen) {
    return;
  }

  Serial.print("Airlock open confirmed by ");
  Serial.print(sourceName);
  Serial.println(". Resuming line following.");

  clearAirlockWait();

  if (robotState == STATE_RFID_PAUSE) {
    lastLineSeenMs = millis();
...
void handleOpenAirlockAck(const String &messageText) {
  ...
  bool airlockMatches = airlock.length() == 0 || airlock == TASK2_AIRLOCK_ID;
  bool openConfirmed = textIsTrue(success) || textIsTrue(accepted);

  if (airlockMatches && openConfirmed) {
    resumeAfterAirlockOpen("openAirlock reply");
  }
}
...
void handleTeamStatusPayload(const uint8_t *payload) {
  ...
  bool taskAirlockOpen = String(TASK2_AIRLOCK_ID) == "B" ? payload[1] == 1 : payload[3] == 1;
  if (taskAirlockOpen) {
    resumeAfterAirlockOpen("team status");
  }
}
...
void printHelp() {
  ...
  Serial.println("RFID read during Task 2 sends openAirlock every 200ms until airlock opens");
  Serial.println("Robot waits stopped at RFID until openAirlock reply or team status confirms the door");
