 ...
 *   UI: Start D32, Kill/stop D53, Red LED D34, Green LED D35
 *
...
const int  FIXED_NORTH_2    = 2;     // 2 nodes forward
...
// D32 starts the staircase mission. D53 requests kill/abort/stop.
...
bool handlePlantCommand(String cmd);
...
bool requireIdleForPlantCommand() {
  if (sysState != SYS_IDLE) {
    Serial.println("Stop Task 4 before manual plant/dropper control.");
    return false;
  }
...
bool handlePlantCommand(String cmd) {
  ...
  Serial.println("Unknown plant command. Use plant drop/load/reset/open/close/status.");
  return true;
}
...
void runFixedPath() {
  ...
  if (isSeedDropperBusy()) {
    Serial.println("Cannot start Task 4 while plant dropper is busy.");
    return;
  }
...
void runSingleHop() {
  if (sysState != SYS_IDLE) return;
  if (isSeedDropperBusy()) {
    Serial.println("Cannot start hop while plant dropper is busy.");
    return;
  }
...
void processCommand(String cmd) {
  ...
  if (handlePlantCommand(cmd)) return;
...
void handleButtons() {
  ...
  bool stopNow  = (digitalRead(BTN_STOP)  == LOW);  // D53 — kill/standalone stop
  ...
  // Rising edge on D53 aborts the current motion.
...
void setup() {
  ...
  Serial.println("  plant drop/load/reset/open/close/status");
