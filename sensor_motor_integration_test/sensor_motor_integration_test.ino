...
void startRobot() {
  ...
  Serial.println("Robot started in full integration mode.");
}
...
void printSnapshot(const LineSnapshot &snapshot) {
  ...
  Serial.print(stateName(robotState));
  Serial.print(" mode=");
  Serial.print(lineOnlyMode ? "LINE_ONLY" : "FULL");
...
void printHelp() {
  Serial.println();
  Serial.println("--- Sensor + motor integration test ---");
  Serial.println("g = start following");
...
void setup() {
  ...
  Serial.println("Sensor + motor integration test starting...");
  ...
  updateStatusLED();
  printHelp();
  Serial.println("Place robot on the line. Send o for line-only mode, or g/D32 for full mode.");
}
