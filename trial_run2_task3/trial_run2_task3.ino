...
const int TURN_SPEED = 580;
const int TURN_BOOST_SPEED = 580;
const int PRE_TURN_SPEED = 580;
...
const bool TASK3_AUTO_DROP_CLOCKWISE = false;
...
int currentDropperAngle = SERVO_RANGE_DEGREES;
int targetDropperAngle = SERVO_RANGE_DEGREES;
...
const char *const TASK3_FIXED_PATH_UIDS[] = {
  "855AAB41",
  "DF54A941",
  "03CCAA41",
  "1D65AA41",
...
const uint8_t TASK3_FIXED_PATH_COUNT = sizeof(TASK3_FIXED_PATH_UIDS) / sizeof(TASK3_FIXED_PATH_UIDS[0]);
...
Heading currentHeading = HEADING_SOUTH;
Heading configuredStartHeading = HEADING_SOUTH;
...
void setupSeedDropper() {
  ...
  currentDropperAngle = SERVO_RANGE_DEGREES;
  targetDropperAngle = SERVO_RANGE_DEGREES;
...
bool handlePlantCommand(const String &lower) {
  ...
  bool isZeroCommand = lower == "plant zero" || lower == "zero" ||
                       lower == "plant servo reset" || lower == "plant reset servo";

  if (!isLoadCommand && !isManualDropCommand && !isZeroCommand && !lower.startsWith("plant ")) {
    return false;
  }
  ...
  if (isLoadCommand) {
    ...
    if (isSeedDropperBusy()) {
      ...
      Serial.print("Plant load servo moved. Seeds recorded: ");
      Serial.println(seedsRemaining);
    } else {
  ...
  if (lower == "plant reset") {
    ...
    if (isSeedDropperBusy()) {
      ...
      currentDropperAngle = SERVO_RANGE_DEGREES;
      targetDropperAngle = SERVO_RANGE_DEGREES;
      Serial.println("Plant seed count reset. Current physical servo position is treated as loaded 300 degrees.");
    }
    return true;
  }
...
int findActiveRouteIndexForPoint(GridPoint point) {
...
bool buildTask3FixedPathRoute() {
  activeRouteLength = 0;
  activeRouteIndex = 0;

  if (TASK3_FIXED_PATH_COUNT > MAX_ROUTE_POINTS) {
    Serial.println("Task3 fixed path is too long.");
    return false;
  }

  for (uint8_t i = 0; i < TASK3_FIXED_PATH_COUNT; i++) {
    GridPoint point;

    if (!findUidInMap(String(TASK3_FIXED_PATH_UIDS[i]), point)) {
      Serial.print("Task3 fixed path UID not found: ");
      Serial.println(TASK3_FIXED_PATH_UIDS[i]);
      activeRouteLength = 0;
      return false;
    }

    activeRoute[activeRouteLength++] = point;
  }

  task3RouteGoal = activeRoute[activeRouteLength - 1];
  return activeRouteLength >= 2;
}
...
bool prepareTask3PlantingMission() {
  ...
  Serial.println("Task3 uses the fixed RFID path and stops at the exit even if fewer than 5 seeds are planted.");

  return buildTask3FixedPathRoute();
}
...
RouteAction nextTask3RouteAction() {
  if (activeRouteLength < 2 || activeRouteIndex >= activeRouteLength - 1) {
    return ROUTE_ACTION_STOP;
  }
...
bool continueTask3NavigationFromCurrent(bool pauseBeforeAction) {
  ...
  int routeMatch = findActiveRouteIndexForPoint(task3CurrentPoint);

  if (routeMatch < 0) {
    Serial.print("Task3 RFID is not on the fixed path: ");
    Serial.println(uidAtPoint(task3CurrentPoint));
    stopRobot();
    return false;
  }

  if (routeMatch < activeRouteIndex) {
    Serial.println("Task3 RFID is behind the fixed path index. Ignoring duplicate/late tag.");
  ...
  if (isTask3ExitPoint(task3CurrentPoint) || activeRouteIndex >= activeRouteLength - 1) {
...
void handleTask3PlantingRfid(const String &uid) {
  ...
  int routeMatch = findActiveRouteIndexForPoint(point);

  if (routeMatch < 0) {
    Serial.println("Task3 RFID is not on the fixed path. Ignoring.");
    return;
  }
  ...
  if (isTask3ExitPoint(point)) {
    completeTask3MissionAtExit();
    return;
  }

  if (isTask3PlantableCell(point) && routeMatch > 0) {
    if (!isTask3CellPlanted(point)) {
      startTask3PlantAtCurrentPoint();
      return;
    }

    Serial.println("Task3 fixed-path cell already planted. Continuing.");
  } else {
    Serial.println("Task3 fixed-path navigation point, no planting here.");
  }
...
void printHelp() {
  ...
  Serial.println("zero / plant zero = move servo to software 0 degrees");
  Serial.println("plant reset/open/close/status = seed dropper controls");
...
void processSerialCommand(String command) {
  ...
  if (command.length() == 1) {
    ...
    if (key == '1') {
      ...
      Serial.println("Debug turn keys are disabled. Use drop/plant drop for seed control.");
...
void handleSerialCommands() {
  while (Serial.available()) {
    ...
    if (input == '\r' || input == '\n') {
      ...
      serialLine += input;
      String lowerBuffered = serialLine;
      lowerBuffered.toLowerCase();

      if (lowerBuffered == "load" || lowerBuffered == "drop" || lowerBuffered == "zero") {
        processSerialCommand(serialLine);
        serialLine = "";
        continue;
      }
